    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6">
        <div class="flex items-center justify-between gap-4">
          ...
          <span class="w-10 h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow-md">
            <!-- Edit icon -->
            <svg viewBox="0 0 24 24" class="w-5 h-5 fill-current" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
          </span>
                <div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight"> ویرایش موشن گرافیک</h1>
    ...
    <section class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800">
          ...
          <span class="inline-flex w-8 h-8 items-center justify-center rounded-lg bg-indigo-600 text-white">
            <svg viewBox="0 0 24 24" class="w-4 h-4 fill-current" aria-hidden="true"><path d="M3 5h18v2H3zm0 6h18v2H3zm0 6h18v2H3z"/></svg>
          </span>
                اطلاعات موشن
            </h2>
        ...
        <!-- Edit Form (no JS) -->
        <form action="{{route('admin.motions.update',$motion->id)}}" method="post" class="px-6 py-6 space-y-6">
            @method('put')
            @csrf
            <!-- Title -->
            <div>
                ...
                <input id="title" name="title" type="text" required
                       value="{{old('title',$motion->title)}}"
                       placeholder="مثلاً: حرکت آهسته آبشار"
                       class="w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 px-4 py-3 text-slate-800 dark:text-slate-100 placeholder-slate-400">
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <div id="dropZone"
                        ...
                        <div class="flex items-stretch space-x-2 gap-2">
                            <input type="text" id="image_label" name="video_url" dir="ltr" value="{{old('video_url',$motion->video_url)}}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 px-4 py-3 text-slate-800 dark:text-slate-100 placeholder-slate-400"
                          placeholder="توضیح کوتاهی درباره ویدئو بنویسید...">{{$motion->description}}</textarea>
            </div>
            ...
            <div class=" gap-4">
                <div>
                    ...
                    <select id="active" name="active"
                            class="w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 px-4 py-3">
                        <option value="1" {{$motion->active ? 'selected':''}}>فعال </option>
                        <option value="0" {{!$motion->active ? 'selected':''}}>غیرفعال </option>
                    </select>
                </div>
            </div>
