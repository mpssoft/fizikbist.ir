...
<div class="container mx-auto px-4 max-w-4xl" >
    ...
    <div class="mt-10 mb-10 relative bg-gradient-to-br from-white/10 via-white/5 to-transparent dark:from-slate-800/20 dark:via-slate-900/10 dark:to-transparent backdrop-blur-md border border-white/30 dark:border-slate-600/30 rounded-3xl shadow-xl overflow-hidden transition-colors duration-300">
            ...
            @foreach($sliders as $slider)
                <div class="slide absolute inset-0 w-full h-full" id="slide-1">
                    <div class="relative  w-full h-full">
                        <!-- Background -->
                        <div class="absolute inset-0 bg-gradient-to-br from-purple-500/10 via-blue-500/5 to-pink-500/10 dark:from-purple-800/20 dark:via-blue-900/10 dark:to-pink-800/20 backdrop-blur-sm"></div>
                        ...
                        <div class="relative z-10 grid grid-cols-1 md:grid-cols-2 h-full md:pr-30" dir="ltr">
                            ...
                            <div dir="rtl" class="hidden md:flex text-right slide-content flex-col justify-center px-8 text-white order-2 md:order-1 z-20">
                                ...
                                @if($slider->link)
                                    <a href="{{$slider->link}}" class="leading-right bg-white/90 dark:bg-slate-800/90 text-blue-600 dark:text-blue-400 px-6 py-3 rounded-full font-semibold hover:bg-white dark:hover:bg-slate-700 transition-colors duration-300 shadow-lg backdrop-blur-sm w-fit">اطلاعات بیشتر</a>
                                @endif
                            </div>
                            <!-- Photo Section (Right) -->

                            <div class="slide-image flex items-center justify-center p-8 md:p-12 order-1 md:order-2 z-20 slide-enter-right">
                                <div class="w-[520px] max-w-[520px] md:w-[520px] md:h-[390px] rounded-2xl overflow-hidden shadow-2xl bg-white/20 dark:bg-slate-800/20 backdrop-blur-sm border border-white/30 dark:border-slate-600/30">
        ...
        <!-- Navigation Arrows -->
        <button class="absolute left-4 top-1/2  transform -translate-y-1/2 bg-white/20 dark:bg-slate-800/30 hover:bg-white/30 dark:hover:bg-slate-700/40 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm border border-white/20 dark:border-slate-600/30 z-30" onclick="previousSlide()">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>

        <button class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/20 dark:bg-slate-800/30 hover:bg-white/30 dark:hover:bg-slate-700/40 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm border border-white/20 dark:border-slate-600/30 z-30" onclick="nextSlide()">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute bottom-6  left-1/2 transform -translate-x-1/2 flex z-30">
            ...
            @foreach($sliders as $s)
                <button class="w-3 h-3 rounded-full bg-white/60 dark:bg-slate-400/60 hover:bg-white dark:hover:bg-slate-200 transition-all duration-500 hover:scale-110 active-dot ml-3 shadow-lg" onclick="goToSlide({{$index++}})" id="dot-0"></button>
            @endforeach
